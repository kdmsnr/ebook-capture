...
- Self timer: 最初の撮影前に待つ秒数。`Sound` を選ぶと最後の3秒で音を鳴らしてカウントダウン
- Sound: セルフタイマーの最後の3秒で鳴らす音。既定は `None`。音を選ぶと `Preview` で試聴

`Capture until book end` をオンにした場合は、ページ数に達するか、ページ送り後に画面が変わらなくなるかの早い方で停止します。終端検出に失敗しても止まるように、`Pages` は実際のページ数より少し多めに設定してください。

`Command+Q` でアプリを終了できます。
...
キャプチャが終了すると、`Ebook Capture.app` のウィンドウを前面に戻します。
