...
電子書籍ビューアの表示中ウィンドウを連続撮影し、ページ送りキーを送る macOS 向けツールです。既定の対象アプリは `Kindle.app` です。
...
`make app` は `dist/Ebook Capture.app` を作成します。アプリは ad-hoc 署名ですが、次の固定署名要件を埋め込むため、再ビルド後も同じアプリとして扱われやすくしています。
...
- App name: 対象アプリ名。既定値は `Kindle.app`
...
```sh
dist/Ebook\ Capture.app/Contents/MacOS/ebook-capture --count 30 --output-dir ./captures --delay 1.0
```
...
-o, --output-dir <directory>  画像の保存先ディレクトリ。既定値: captures
    ...
    --app-name <name>         macOS アプリ名。既定値: Kindle.app
